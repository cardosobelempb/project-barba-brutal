...
export class EmailVO {
  ...
  private constructor(value: string, label?: string, private readonly options?: EmailOptions) {
    ...
    if (!EmailVO.isValid(normalized)) {
      ...
      throw new BadRequestError(t('errors.email.invalid', { args: { email: value } }))
    }
  ...
  private static defaultMessages(key: string, options?: { args?: Record<string, any> }): string {
