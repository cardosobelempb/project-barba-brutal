...
export class PhoneVO {
  ...
  public static create(
    ...
    const { minLength = this.DEFAULT_MIN_LENGTH, maxLength = this.DEFAULT_MAX_LENGTH } = options
  ...
  private static validate(cleaned: string, min: number, max: number): void {
    if (!/^\d+$/.test(cleaned)) {
      throw new BadRequestError(`Telefone contém caracteres inválidos: ${cleaned}`)
    }
